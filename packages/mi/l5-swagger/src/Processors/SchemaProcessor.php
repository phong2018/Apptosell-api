...
class SchemaProcessor extends AbstractProcessor
{
    public static function ruleToSchema($key, $rule, $allRules = [])
    {
        ...
        if (preg_match(self::REGEX_INT, $rule)) {
            return self::toIntSchema($rule);
        }
    ...
    protected static function toIntSchema($rule)
    {
        $type   = self::ruleToType('integer');
        $schema = new $type([
        ...
        foreach ($allRules as $k => $v) {
            if (preg_match('/' . $key . '\.\*\.(\w+)/', $k, $matches)) {
                ...
                if (preg_match(self::REGEX_ARRAY, $v)) {
                    $allowKey = $k;
                    $rule = self::ruleToSchema($matches[1], $v, $allRules);
